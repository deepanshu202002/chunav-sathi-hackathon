...
export default function Timeline({ steps, onSelectStep, currentStep = 1 }: TimelineProps) {
  return (
    <div className="relative w-full max-w-full overflow-hidden p-4 lg:p-6">
      ...
      <div className="flex lg:hidden overflow-x-auto pb-4 space-x-4 snap-x snap-mandatory no-scrollbar">
        {steps.map((step) => (
          <motion.button
            key={step.id}
            onClick={() => onSelectStep(step.query)}
            className={cn(
              "flex-shrink-0 flex items-center space-x-2 px-4 py-2 rounded-full snap-start border transition-all",
